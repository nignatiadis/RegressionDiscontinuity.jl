...
function density_test(
    ...
    if plot
        ...
        lVarf =
            (12 / (5 * n * h)) .* ldf.Y .* (
                2.0 .+ 3 * m .^ 11 .- 24 * m .^ 10 .+ 83 * m .^ 9 .- 72 * m .^ 8 .-
                42 * m .^ 7 .+ 18 * m .^ 6 .+ 18 * m .^ 5 .+ 18 * m .^ 4 .+ 3 * m .^ 3 .+
                18 * m .^ 2 .+ 15 * m
            ) ./ ((
                1.0 .+ m .^ 6 .- 6 * m .^ 5 .- 3 * m .^ 4 .+ 4 * m .^ 3 .+ 9 * m .^ 2 .+
                6 * m
            ))
        ...
        plot_final = scatter(
            ...
            Y,
            leg = false,
            palette = :cyclic_grey_15_85_c0_n256,
            alpha = 0.5,
            title = "Density Test",
        ...
        plot!(
            ...
            alpha = 0.55,
            palette = :cyclic_grey_15_85_c0_n256,
            ribbon = (ldf.CIup .- ldf.pred, ldf.pred .- ldf.CIlow),
        )
        plot!(
            rdf.X,
            rdf.pred,
            ...
            alpha = 0.55,
            palette = :cyclic_grey_15_85_c0_n256,
            ribbon = (rdf.CIup .- rdf.pred, rdf.pred .- rdf.CIlow),
        )
    ...
    lfhat = sum(
        tri_kernel.((ldf.X .- c) ./ h) .* (
            (Sminus(2) .- Sminus(1) .* (ldf.X .- c)) ./
            (Sminus(2) * Sminus(0) - (Sminus(1))^2)
        ) .* ldf.Y,
    )
    ...
    rfhat = sum(
        tri_kernel.((rdf.X .- c) ./ h) .* (
            (Splus(2) .- Splus(1) .* (rdf.X .- c)) ./ (Splus(2) * Splus(0) - (Splus(1))^2)
        ) .* rdf.Y,
    )
    ...
    if verbose
        if bin === nothing
            println(
                "Using default bin size calculation, bin size:   ",
                round(b, digits = 4),
            )
        else
            println(
                "Using bin size given, bin size:                 ",
                round(b, digits = 4),
            )
        end
    end
    ...
    if verbose
        if bw === nothing
            println(
                "Using default bandwidth calculation, bandwidth: ",
                round(h, digits = 4),
            )
        else
            println(
                "Using bandwidth given, bandwidth:               ",
                round(h, digits = 4),
            )
        end
    end
    ...
    if generate
        gen = DataFrame(
            Y = Y,
            X = X,
            fhat = vcat(ldf.pred, rdf.pred),
            se_fhat = vcat(ldf.se_pred, rdf.se_pred),
        )
        if plot
            return pval, plot_final, θhat, σθ, b, h, gen
        else
